...
const querySchema=z.object({modelId:z.string().min(1).max(300).optional(),range:z.enum(['1D','3D','1W','1M','3M','6M','1Y']).default('1M'),timezone:z.string().min(1).max(80).default('UTC'),search:z.string().max(100).optional(),cursor:z.string().datetime({offset:true}).optional(),type:z.string().max(80).optional()});
...
async function legacyMode(userId:string,modelId?:string){
  const admin=getSupabaseAdmin();const profile=await admin.from('profiles').select('platform_role,status').eq('id',userId).maybeSingle();
  if(profile.data?.platform_role!=='platform_admin'||profile.data.status!=='active')return {enabled:false,girlId:null};
  if(modelId?.startsWith('legacy:'))return {enabled:true,girlId:modelId.slice(7)};
  const normalized=await allowedModels(userId);return {enabled:!modelId&&normalized.length===0,girlId:null};
}

async function getLegacyEvents(start:string,end:string,girlId:string|null){
  let query=getSupabaseAdmin().from('dashboard_events').select('id,girl_id,girl_name,user_id,username,event_type,stars,created_at').gte('created_at',start).lt('created_at',end).order('created_at',{ascending:false}).limit(10000);
  if(girlId)query=query.eq('girl_id',girlId);return query;
}
...
dashboardRouter.get('/context',async(req:AuthenticatedRequest,res:Response)=>{
  ...
  let availableModels:any[]=models.data||[];
  if(isAdmin){const legacy=await admin.from('dashboard_events').select('girl_id,girl_name').limit(10000);if(!legacy.error){const unique=new Map<string,string>();legacy.data?.forEach(row=>unique.set(row.girl_id,row.girl_name));availableModels=[...availableModels,...[...unique].map(([id,name])=>({id:`legacy:${id}`,display_name:name,currency:'USD',status:'active'}))];}}
  return res.json({success:true,account_type:accountType,account_label:isAdmin?'Admin account':hasAgency?(agencyRow?.agencies?.name||'Agency account'):'Model account',can_combine:accountType!=='model',models:availableModels});
});
...
dashboardRouter.get('/summary',async(req:AuthenticatedRequest,res:Response)=>{
  ...
  const {start,end}=bounds(parsed.data.range);const legacy=await legacyMode(req.userId!,parsed.data.modelId);if(legacy.enabled){const events=await getLegacyEvents(start,end,legacy.girlId);if(events.error)return fail(res,503,'summary_unavailable','Unable to load earnings summary.');const rows=events.data||[],purchases=rows.filter(row=>row.event_type==='star_purchase');return res.json({success:true,summary:{total_stars:purchases.reduce((sum,row)=>sum+Number(row.stars),0),total_revenue:0,total_sales:purchases.length,unique_buyers:new Set(purchases.map(row=>row.user_id)).size,messages:rows.filter(row=>row.event_type==='message').length,total_users:new Set(rows.map(row=>row.user_id)).size,active_users:new Set(rows.map(row=>row.user_id)).size,model_count:new Set(rows.map(row=>row.girl_id)).size,role:'agency',title:legacy.girlId?`${rows[0]?.girl_name||'Model'} earnings`:'Combined earnings'}});}
  const modelIds=await scope(req,res,parsed.data.modelId);if(!modelIds)return;const admin=getSupabaseAdmin();
...
dashboardRouter.get('/revenue-history',async(req:AuthenticatedRequest,res:Response)=>{
  const parsed=querySchema.safeParse(req.query);if(!parsed.success)return fail(res,400,'invalid_query','Invalid dashboard filters.');const {start,end}=bounds(parsed.data.range);const legacy=await legacyMode(req.userId!,parsed.data.modelId);if(legacy.enabled){const events=await getLegacyEvents(start,end,legacy.girlId);if(events.error)return fail(res,503,'history_unavailable','Unable to load history.');const grouped=new Map<string,{metric_date:string,revenue:number,stars:number}>();for(const row of events.data||[]){const date=row.created_at.slice(0,10);const item=grouped.get(date)||{metric_date:date,revenue:0,stars:0};if(row.event_type==='star_purchase')item.stars+=Number(row.stars);grouped.set(date,item);}return res.json({success:true,history:[...grouped.values()].reverse()});}const modelIds=await scope(req,res,parsed.data.modelId);if(!modelIds)return;const admin=getSupabaseAdmin();
...
dashboardRouter.get('/transactions',async(req:AuthenticatedRequest,res:Response)=>{
  const parsed=querySchema.safeParse(req.query);if(!parsed.success)return fail(res,400,'invalid_query','Invalid dashboard filters.');const {start,end}=bounds(parsed.data.range);const legacy=await legacyMode(req.userId!,parsed.data.modelId);if(legacy.enabled){const events=await getLegacyEvents(start,end,legacy.girlId);if(events.error)return fail(res,503,'transactions_unavailable','Unable to load transactions.');const rows=(events.data||[]).filter(row=>row.event_type==='star_purchase').slice(0,50);return res.json({success:true,transactions:rows.map(row=>({id:String(row.id),user:row.username||row.user_id,model:row.girl_name,stars:row.stars,revenue:0,currency:'USD',type:row.event_type,created_at:row.created_at})),next_cursor:null});}const modelIds=await scope(req,res,parsed.data.modelId);if(!modelIds)return;const admin=getSupabaseAdmin();let query=admin.from('transactions').select('id,model_id,source_buyer_id,stars,gross_amount_cents,currency,transaction_type,occurred_at,models(display_name)').in('model_id',modelIds).gte('occurred_at',start).lt('occurred_at',end).eq('transaction_status','completed').order('occurred_at',{ascending:false}).limit(51);
...
dashboardRouter.get('/sync-health',async(req:AuthenticatedRequest,res:Response)=>{
  const parsed=querySchema.safeParse(req.query);if(!parsed.success)return fail(res,400,'invalid_query','Invalid dashboard filters.');const legacy=await legacyMode(req.userId!,parsed.data.modelId);if(legacy.enabled){let query=getSupabaseAdmin().from('dashboard_events').select('created_at').order('created_at',{ascending:false}).limit(1);if(legacy.girlId)query=query.eq('girl_id',legacy.girlId);const {data,error}=await query.maybeSingle();if(error)return fail(res,503,'sync_health_unavailable','Unable to load synchronization status.');if(!data)return res.json({success:true,status:'never_synced',last_successful_sync_at:null});const minutes=Math.floor((Date.now()-new Date(data.created_at).getTime())/60000);return res.json({success:true,last_successful_sync_at:data.created_at,minutes_since_success:minutes,status:minutes<=45?'fresh':minutes<=60?'delayed':'stale'});}const modelIds=await scope(req,res,parsed.data.modelId);if(!modelIds)return;const admin=getSupabaseAdmin();const {data,error}=await admin.from('sync_runs').select('completed_at').in('model_id',modelIds).eq('status','succeeded').order('completed_at',{ascending:false}).limit(1).maybeSingle();if(error)return fail(res,503,'sync_health_unavailable','Unable to load synchronization status.');if(!data?.completed_at)return res.json({success:true,status:'never_synced',last_successful_sync_at:null});const minutes=Math.floor((Date.now()-new Date(data.completed_at).getTime())/60000);return res.json({success:true,last_successful_sync_at:data.completed_at,next_expected_sync_at:new Date(new Date(data.completed_at).getTime()+1800000).toISOString(),minutes_since_success:minutes,status:minutes<=45?'fresh':minutes<=60?'delayed':'stale'});
});

dashboardRouter.get('/models',async(req:AuthenticatedRequest,res:Response)=>{
  const parsed=querySchema.safeParse(req.query);if(!parsed.success)return fail(res,400,'invalid_query','Invalid dashboard filters.');const {start,end}=bounds(parsed.data.range);const legacy=await legacyMode(req.userId!,parsed.data.modelId);if(legacy.enabled){const events=await getLegacyEvents(start,end,legacy.girlId);if(events.error)return fail(res,503,'models_unavailable','Unable to load models.');const grouped=new Map<string,{id:string,display_name:string,total_revenue:number,total_stars:number}>();for(const row of events.data||[]){const item=grouped.get(row.girl_id)||{id:`legacy:${row.girl_id}`,display_name:row.girl_name,total_revenue:0,total_stars:0};if(row.event_type==='star_purchase')item.total_stars+=Number(row.stars);grouped.set(row.girl_id,item);}return res.json({success:true,models:[...grouped.values()]});}const modelIds=await scope(req,res,parsed.data.modelId);if(!modelIds)return;const admin=getSupabaseAdmin();const [models,tx]=await Promise.all([admin.from('models').select('id,display_name,slug,profile_image_url,currency').in('id',modelIds),admin.from('transactions').select('model_id,gross_amount_cents,stars').in('model_id',modelIds).gte('occurred_at',start).lt('occurred_at',end).eq('transaction_status','completed').limit(10000)]);if(models.error||tx.error)return fail(res,503,'models_unavailable','Unable to load models.');const totals=new Map<string,{revenue:number,stars:number}>();for(const row of tx.data||[]){const item=totals.get(row.model_id)||{revenue:0,stars:0};item.revenue+=Number(row.gross_amount_cents)/100;item.stars+=Number(row.stars);totals.set(row.model_id,item);}return res.json({success:true,models:(models.data||[]).map(model=>({...model,total_revenue:totals.get(model.id)?.revenue||0,total_stars:totals.get(model.id)?.stars||0}))});
});
